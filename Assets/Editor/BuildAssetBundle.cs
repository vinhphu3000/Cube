...
namespace Assets.Editor
{
    public class BuildAssetBundle : UnityEditor.Editor
    {
        ...
        static void BuildAllAssetBundle()
        {
            ...
            BuildUIAssetBundle("/Res/Prefab", "Assets/StreamingAssets/ui", CurrentBuildTarget());
        }

        static void BuildUIAssetBundle(string selectPath, string outputPath, BuildTarget buildTarget)
        {
            //获取路径下的prefab
            Dictionary<string, List<string>> mainUIPrefabs = GetPrefabGroupInPath(Application.dataPath + selectPath);
